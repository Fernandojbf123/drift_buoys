...
def Gra_series_de_tiempo_telemetria(dataFrame: pd.DataFrame, 
                                    ...
                                    config: ProcesadoConfig | None = None) -> tuple[Figure, str]:
    """
    Descripción:
    ...
    def tamanio_de_figura(cantidad_de_vars = len(config.variables_a_graficar)):
        output = cantidad_de_vars * 2
        return (15,output)
    
    fig, axes = plt.subplots(nrows=len(config.variables_a_graficar), figsize=tamanio_de_figura(len(config.variables_a_graficar)))
    axes = np.atleast_1d(axes)  # Asegurar que axes es siempre un array 1D
    
    # Variables a graficar
    var_names = config.variables_a_graficar
    ...
    titulo_de_figura, nombre_de_guardado = crear_titulo_de_figura_serie_de_tiempo_y_nombre_de_guardado(tspan=tspan, NS_sonda=NS_sonda, config=config)
