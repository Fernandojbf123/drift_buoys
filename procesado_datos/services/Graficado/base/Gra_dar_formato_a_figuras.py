...
##################################################################################
def dar_formato_a_figura_de_series_de_tiempo(propiedades: dict, config) -> None:
    """
    Descripción:
    ...
    tipo_de_letra = config.tipo_de_letra
    titlesize = config.tamanio_de_letra + 6
...
def dar_formato_al_axe(propiedades_del_axe: dict, config) -> None:
    """
    Descripción:
    ...
    tipo_de_letra = config.tipo_de_letra
    tamanio_de_letra = config.tamanio_de_letra
...
def crear_titulo_de_figura_serie_de_tiempo_y_nombre_de_guardado(tspan: pd.DatetimeIndex, NS_sonda: str, config: ProcesadoConfig) -> tuple:
    origen_de_los_datos = config.origen_de_los_datos
...
def dar_formato_al_mapa(propieadades_de_mapa, config) -> None:
    """
    Descripción:
    ...
    tipo_de_letra = config.tipo_de_letra # fontfamily
    tamanio_de_letra = config.tamanio_de_letra
    ...
    if obj_mapeable is not None:
        cb = plt.colorbar(obj_mapeable, label=colorbar_label, orientation='vertical', pad=0.02, aspect=30, ticks=np.round(np.linspace(minimo, maximo, 7), config.decimales_en_figuras))
...
def crear_titulos_de_mapa_y_nombre_de_guardado(tspan: pd.DatetimeIndex, NS_sonda: str, config) -> dict:
    """
    Descripción:
    ...
    nombre_de_guardado = f"mapa_trayectoria_{NS_sonda}"
