...
from global_config.manager_diccionario_variables import *
from procesado_datos.config_modulo.ProcesadoConfig import ProcesadoConfig

#################### FUNCIONES #########################

def buscar_nombre_de_archivo_de_sonda(config) -> tuple:
    ...
    """
    carpeta_de_datos_crudos = crear_ruta_a_carpeta(config.get("carpeta_de_datos_crudos"))
...
def leer_excel_de_despliegue_de_sondas(config) -> pd.DataFrame:
    ...
    Estructura del DataFrame de salida :
        ...
        campania = string
    """
    ruta_al_excel_de_despliegue_de_sondas = config.ruta_al_excel_de_despliegue_de_sondas
...
def seleccionar_rango_de_fechas(diccionario: dict, 
                                buscar_fechas_anteriores_al_estudio: bool = False,
                                config: ProcesadoConfig | None = None)-> dict:
    ...
    Parámetros:
        ...
        config (ProcesadoConfig | None): Configuración del análisis.
    ...
    fecha_de_inicio_del_analisis = config.convertir_a_pd_datetime("fecha_de_inicio_del_analisis", formato="%Y-%m-%d %H:%M")
    fecha_de_fin_del_analisis = config.convertir_a_pd_datetime("fecha_de_fin_del_analisis", formato="%Y-%m-%d %H:%M")
    ...
    for serial in list(diccionario.keys()):
        ...
        if mask.empty:
            msg = f"La sonda {serial} no tiene datos en el rango de fechas {config.fecha_de_inicio_del_analisis}-{config.fecha_de_fin_del_analisis}"
