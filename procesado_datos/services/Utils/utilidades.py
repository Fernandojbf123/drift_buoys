...
def calcular_porcentaje_de_datos_recibidos(diccionario: dict, config) -> dict:
    ...
    for iserial, serial in enumerate(seriales_de_sondas):
        ...
        try:
            ...
            tspan_synth = crear_rango_de_fechas_sintetico(fecha_de_inicio = fecha_de_inicio_redondeada[0], fecha_de_fin = fecha_de_final_redondeada[0], delta_tiempo = config.delta_tiempo)
...
def cargar_datos_de_batimetria(config) -> dict:
    ...
    datos_batimetria = {
        ...
        "lat": None,
        "elevation": None,
        "curvas_de_batimetria": config.curvas_de_batimetria
    }
    
    ruta = config.ruta_a_datos_batimetria
...
def cargar_datos_de_topografia(config) -> dict:
    ...
    ruta = config.ruta_a_datos_topografia
...
def guardar_diccionario_como_pickle(diccionario: dict, ruta: str, nombre_archivo: str) -> None:
    ...
    ruta (str): Ruta a la carpeta donde se guardará el archivo pickle.
